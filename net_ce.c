...
float crossEntropyLoss(float x[][outputLayerSize], int y[][outputLayerSize], int m)
{
    float sum = 0 ;
    for(int i = 0; i < outputLayerSize; i++)
    {
        sum -= y[m][i]*log(x[m][i]) + (1 - y[m][i])*log(1 - x[m][i]);
    }
    return sum;
}
...
void train(int opt)
{
    ...
    for(long int epoch = 0; epoch < limit; epoch++ )
    {
        ...
        for(int m = 0; m < trainingSetSize; m++ )
        {
            ...
            error = crossEntropyLoss(outputLayer, target, m);
            /*sum = 0;
            for(int i =0 ; i < outputLayerSize; i++)
            {
                sum -= target[m][i]
           ...
           /* for(int i = 0; i < outputLayerSize; i++)
            {
                outputDelta[i] = (float)(outputError[i] * outputLayer[m][i] * (1 - outputLayer[m][i]));
            }*/
